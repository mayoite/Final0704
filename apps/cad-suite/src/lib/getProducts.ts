...
import { fetchCloudflareBackupProducts } from "./cloudflareBackupCatalog";
...
async function fetchAllProductsLive(): Promise<Product[]> {
    ...
    if (error) {
        ...
        const backupProducts = await fetchCloudflareBackupProducts();
        if (backupProducts && backupProducts.length > 0) {
            return backupProducts.map((p) => ({
                ...p,
...
async function fetchProductsByCategoryLive(categoryId: string): Promise<Product[]> {
    ...
    if (error) {
        ...
        const backupProducts = await fetchCloudflareBackupProducts({ categoryId });
        if (backupProducts && backupProducts.length > 0) {
            return backupProducts.map((p) => ({
                ...p,
                images: normalizeAssetList(p.images),
...
async function fetchProductByUrlKeyLive(productUrlKey: string): Promise<Product | null> {
    ...
    if (error) {
        ...
        const backupProducts = await fetchCloudflareBackupProducts({ productUrlKey });
        if (backupProducts && backupProducts.length > 0) {
            const backupProduct = backupProducts[0];
            return {
                ...backupProduct,
                images: normalizeAssetList(backupProduct.images),
                flagship_image: normalizeAssetPath(backupProduct.flagship_image),
                "3d_model": normalizeAssetPath(backupProduct["3d_model"]),
            };
        }
        return null;
    }
...
async function buildCatalogLive(): Promise<CompatCategory[]> {
    ...
    if (categories.length === 0 && products.length === 0 && lastError) {
        ...
        if (backupProducts && backupProducts.length > 0) {
            ...
            const categoryIds = [...new Set(backupProducts.map((p) => p.category_id).filter(Boolean))];
...
export async function getCategoryIds(): Promise<string[]> {
    ...
    if (error) {
        ...
        const backupProducts = await fetchCloudflareBackupProducts();
        if (!backupProducts || backupProducts.length === 0) return [];
        return [...new Set(backupProducts.map((p) => p.category_id).filter(Boolean))];
    }
