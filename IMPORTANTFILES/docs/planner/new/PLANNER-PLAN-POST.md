...
The planner should be consolidated under `apps/cad-suite`.
...
The donor snapshot in `07docs/Backupcad` should not become the primary planner product.
...
2. One planner code boundary: `apps/cad-suite`
...
- a separate planner-managed product write store
- planner-managed product writes routed only to that new store
- one merged adapter across legacy-read and new-write product sources

## What We Port

### From `07docs/Backupcad`
...
1. Keep `07docs/Backupcad` as the only donor snapshot
...
|---|---|---|
| `react-konva` and `konva` | No live usage found in `apps/cad-suite/src` | Secondary 2D canvas tooling or overlays if Tldraw is not the right fit for a sub-surface |
| `@thi.ng/geom-hull` | No live usage found in `apps/cad-suite/src` | Polygon hull generation and room-outline cleanup |
| `alpha-shape` | No live usage found in `apps/cad-suite/src` | Polygon boundary and enclosure calculations |
| `bezier-js` | No live usage found in `apps/cad-suite/src` | Curves, rounded corners, and wall-path calculations |
| `line-intersect` | No live usage found in `apps/cad-suite/src` | Wall joins, segment intersections, and measurement rules |
| `@tanstack/react-query` | No live planner usage found in `apps/cad-suite/src` | Planner async data synchronization if persistence surfaces grow |
| `html2canvas` | No live usage found in `apps/cad-suite/src` | Planner image snapshots and raster export capture |
| `jspdf` | No live usage found in `apps/cad-suite/src` | PDF export, quote summaries, and printable planner output |
| `jszip` | No live usage found in `apps/cad-suite/src` | Bundled export packages and planner asset archives |
| `axios` | No live usage found in `apps/cad-suite/src` | External planner API integration if native `fetch` is not enough |
| `point-in-polygon` | No live usage found in `apps/cad-suite/src` | Polygon containment checks and layout validation rules |
...
Make `apps/cad-suite` build, Cloudflare-build, and deploy consistently.
...
The current failure pattern points to workspace and Turbopack resolution problems in `apps/cad-suite`.
...
Port selectively from `07docs/Backupcad`.
...
Reuse only the useful parts from `07docs/Backupcad`.
...
1. `apps/cad-suite` production build passes
2. `apps/cad-suite` Cloudflare build passes
...
2. Do not promote `07docs/Backupcad` wholesale into production.
...
- port persistence and 3D selectively from `07docs/Backupcad`
- remove duplication
