...
export function Navbar() {
  ...
  return (
    <>
      <header
        className={cn(
          "fixed top-0 left-0 z-50 w-full border-b border-soft/70 surface-overlay-92 backdrop-blur-xl transition-all duration-300",
          ...
          <div className="nav-shell flex h-20 items-center justify-between px-3 sm:px-4">
            ...
            <div className="flex items-center gap-2 md:gap-3">
                ...
                <div ref={searchPanelRef} className="relative hidden xl:block">
                  ...
                  <AnimatePresence>
                    {showSearchPanel && (
                      ...
                      >
                        <div className="mb-2 flex items-center justify-between typ-caption-lg font-semibold uppercase tracking-widest text-muted">
                          <span>{searchSectionTitle}</span>
                          {searchSource && (
                            <span className="rounded-full bg-hover px-2 py-0.5 typ-caption">
                              {searchSource === "ai"
                                ? "AI Ranked"
                            ...
                            {searchResults.map((result) => (
                              <li key={result.id}>
                                ...
                                >
                                  ...
                                  <span className="typ-caption uppercase tracking-widest text-subtle">
                                    {result.type}
                                  </span>
                                </Link>
                              </li>
              ...
              >
                ...
                {totalQty > 0 && (
                  <span className="absolute -right-1 -top-1 inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 typ-caption font-bold text-inverse">
        ...
        <AnimatePresence>
          {activeDesktopMenu === "Products" && (
            <motion.div
              ...
              onMouseLeave={() => setActiveDesktopMenu(null)}
              className="hidden lg:block border-t border-soft surface-overlay-95 backdrop-blur-xl"
            >
              <div className="container-wide py-8">
                <div className="grid grid-cols-5 gap-5">
                  {groupedCategories.map((group) => (
                    <div key={group.groupId} className="rounded-2xl border border-soft bg-panel p-4">
                      <p className="mb-2 typ-caption-lg font-bold uppercase tracking-widest text-muted">
                        {group.groupLabel}
                      </p>
                      <ul className="space-y-1">
                          ...
                          <li key={item.id}>
                            ...
                            >
                              ...
                              {typeof item.count === "number" && (
                                <span className="typ-caption text-subtle">{item.count}</span>
                              )}
                            </Link>
                                  ...
                                  <li key={`${item.id}-${subcategory.id}`}>
                                    ...
                                    >
                                      ...
                                      {typeof subcategory.count === "number" && (
                                        <span className="typ-caption text-subtle">
                                          {subcategory.count}
                                        </span>
                                      )}
                                    </Link>
          ...
          >
            ...
            {(showSearchPanel || searchQuery.trim().length >= 2) && (
              <div className="mb-5 rounded-2xl border border-soft bg-hover p-3">
                <p className="mb-2 typ-caption font-bold uppercase tracking-widest text-muted">
                  {searchSectionTitle}
                </p>
                {searchLoading ? (
            ...
            <div className="mt-6 grid grid-cols-2 gap-2">
              {NAV_RESOURCE_LINKS.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="menu-chip flex min-h-11 items-center justify-center rounded-full border border-soft px-3 text-xs font-semibold uppercase tracking-wider text-body"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="mt-6 flex gap-2">
              ...
              >
                ...
                Cart
                {totalQty > 0 && (
                  <span className="rounded-full bg-primary px-1.5 py-0.5 typ-caption font-bold text-inverse">
                    {Math.min(totalQty, 99)}
                  </span>
                )}
              </Link>
              <Link
                href="/contact"
                onClick={() => setIsMobileMenuOpen(false)}
                className="flex flex-1 items-center justify-center rounded-full bg-primary py-3 text-sm font-bold uppercase tracking-wider text-inverse"
              >
                Request Quote
              </Link>
            </div>
